...
function renderAuctionTable() {
  ...
  tbody.innerHTML = pageRows.map(row => {
    ...
    return `<tr data-id="${row.id}" class="${auctionSelected.has(row.id) ? 'row-selected' : ''}">
      ...
      <td class="col-addr" title="${esc(row.address)}">${row.address ? `<a href="https://map.naver.com/v5/search/${encodeURIComponent(row.address)}" target="_blank" rel="noopener" class="naver-land-link" title="네이버 지도">N</a> ` : ''}${esc(row.address)}</td>
